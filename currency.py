...
APIKEY = 'fP1Vqg5hwhsow8in7ncAHGFgt5MwmP4nGXyzBTchKD4e'
...
def before_space(s):
    ...
    'The string '+repr(s)+' does not have a space.'

    first_space = introcs.find_str(s, ' ')
    result = s[:first_space]
    return result
...
def after_space(s):
    ...
    """
    assert type(s) == str, repr(s)+' is not a string.'
    assert introcs.count_str(s, ' ') >= 1, 'The string '+repr(s)+\
...
def first_inside_quotes(s):
    ...
    """
    assert type(s) == str, repr(s)+' is not a string.'
    assert introcs.count_str(s, '"') >= 2, 'The string '+repr(s)+\
    ' does not have two double quotes.'

    pos1 = introcs.find_str(s, '"') + 1
    pos2 = introcs.find_str(s, '"', pos1)
    result = s[pos1:pos2]
    return result
...
def get_src(json):
    ...
    pos1 = introcs.find_str(json, '"src"')
    colon = introcs.find_str(json, ':', pos1)
    result = first_inside_quotes(json[colon:])
    return result
...
def get_dst(json):
    ...
    pos1 = introcs.find_str(json, '"dst"')
    colon = introcs.find_str(json, ':', pos1)
    result = first_inside_quotes(json[colon:])
    return result
...
def has_error(json):
    ...
    pos1 = introcs.find_str(json, '"error"')
    colon = introcs.find_str(json, ':', pos1)
    error = first_inside_quotes(json[colon:])
    result = len(error) != 0
    return result
...
def service_response(src, dst, amt):
    ...
    q = f'https://ecpyfac.ecornell.com/python/currency/fixed?src={src}' +\
    f'&dst={dst}&amt={amt}&key={APIKEY}'
...
def exchange(src, dst, amt):
    ...
    exchange_str = get_dst(json)
    exchange_value = before_space(exchange_str)
    result = float(exchange_value)
    return result